...
class ArticleController extends AbstractController
{
    ...
    public function index(ArticleRepository $articleRepository,ArtisteRepository $artisteRepository ,AdminRepository $adminRepository ,CategorieRepository $categorieRepository,Request $request,SessionInterface $session): Response
    {
...
//        $artiste=$artisteRepository->findBy(array('username'=>'mou'))[0];
        ...
        $admin=$adminRepository->findOneBy(["username" => $session->get("usernameadmin")]);
    ...
    #[Route('/adminarticles', name: 'app_article_indexadmin', methods: ['GET', 'POST'])]
    public function indexadmin(ArticleRepository $articleRepository,ArtisteRepository $artisteRepository ,AdminRepository $adminRepository ,CategorieRepository $categorieRepository,Request $request,SessionInterface $session): Response
    {
        # dd($articleRepository->findAll());
        $categories = $categorieRepository->findAll();
//        $session->get("usernameariste");
//        $artiste=$artisteRepository->findBy(array('username'=>'mou'))[0];
    ...
    public function articlefront(
        ...
        ManagerRegistry $mr,ClientRepository $clientRepository, SessionInterface $session
        ...
        return $this->render('article/indexfront.html.twig', [
            ...
            'Artiste' =>$client ,
            'categories' => $categories,
            'nbr' => $favoriRepository->findBy(['id_user' => $client]),
            'keyword' => $keyword,
            'Categorie' => $categorie
    ...
    public function articlefavori(ArticleRepository $articleRepository,PaginatorInterface $paginator,ArtisteRepository $artisteRepository , CategorieRepository $categorieRepository,Request $request, FavoriRepository  $favoriRepository ,ClientRepository $clientRepository, SessionInterface $session): Response
    {
        # dd($articleRepository->findAll());
        $categories = $categorieRepository->findAll();
        $client = new Client();
        ...
        #dd($artiste);
        $article=new Article();
        $article=$favoriRepository->findBy(array('id_user'=>$clientRepository->findOneBy(["username" => $session->get("username")])));
        if($request->isMethod("POST"))
        {
            $minamount = $request->get('minamount');
            $maxamount = $request->get('maxamount');
            ...
            $article=$articleRepository->findByPriceRange((float)substr($minamount, 1),(float)substr($maxamount, 1));
            $articleQuery = $articleRepository->createQueryBuilder('a');
    ...
    public function indexdescription(ArticleRepository $articleRepository, ArtisteRepository $artisteRepository , CategorieRepository $categorieRepository, ManagerRegistry $doctrine, Request $request, $id, RatingRepository $ratingRepository,FavoriRepository $favoriRepository,PaginatorInterface $paginator,ClientRepository $clientRepository, SessionInterface $session): Response
    {
